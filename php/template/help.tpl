...
<p>NicoKakoは、ニコニコ生放送の過去番組をタイムシフト機能でダウンロードするためのウェブアプリです。</p>
<p>次の3ステップで利用が可能です。</p>

<ol>
	<li>番組を登録する<ul>
		<li><a href="<?= h($this->get_url('register')) ?>">register</a>から番組を登録します。</li>
		<li>登録に必要なものは、番組のURLだけです。</li>
		<li>登録前にタイムシフト予約をする必要があります。</li>
	</ul></li>
	<li>ダウンロードを待つ<ul>
		<li>NicoKakoが自動的に番組情報を取得し、ダウンロードを開始します。</li>
		<li>完了までに時間が掛かるので、しばらく待ちます。</li>
	</ul></li>
	<li>番組を閲覧する<ul>
		<li><a href="<?= h($this->config["application_url"]) ?>">index</a>からダウンロードが完了した番組を閲覧できます。</li>
...
<div id="index" class="page-header">
	<h2>index <small>番組を閲覧する</small></h2>
</div>

<ol>
	<li><a href="<?= h($this->config["application_url"]) ?>">index</a>にアクセスする。<ul>
		<li>画面下部に利用可能な番組のリストが表示されます。</li>
	</ul></li>
	<li>「lives」の中から好きな番組のサムネイルをクリックする。<ul>
		<li>プレイヤーの再生ボタンをクリックすると再生が開始されます。</li>
	</ul></li>
</ol>

<div id="register" class="page-header">
	<h2>register <small>番組を登録する</small></h2>
</div>
...
<ol>
	...
	<li>入力欄に登録したい番組のURLを入力して「確認する」をクリックする。<ul>
		<li>URLが正しければ、番組のタイトルなどが抽出され、画面に表示されます。</li>
