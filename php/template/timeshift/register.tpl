...
<div class="page-header">
	<h1><?= h($this->get_title()) ?> <small>タイムシフトの予約</small></h1>
</div>
...
<?php
	...
	if (isset($this->post["confirm"])) {
		if ($is_valid) {
			$this->include_template("timeshift/register_confirm.tpl");
		} else {
			$this->include_template("timeshift/register_default.tpl");
		}
	} else if (isset($this->post["submit"])) {
		$this->include_template("timeshift/register_submit.tpl");
	} else {
		$this->include_template("timeshift/register_default.tpl");
	}
?>
